...
def load_network(network, name):
    ...
    ignore_layers = ['classifier.classifier.0.weight', 'classifier.classifier.0.bias']
    pretrained_dict = {k: v for k, v in pretrained_dict.items() if k not in ignore_layers}
...
hyperparameters['teacher_w'] * self.loss_teacher )  )
    ...
    def resume(self, checkpoint_dir, hyperparameters):
        ...
        #print([k for k, _ in state_dict['a'].items()])
        #print(self.gen_a)
        pretrained_dict = state_dict['a']
        model_dict = self.gen_a.state_dict()
        # 1. filter out unnecessary keys
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if 'nonloc' not in k.split('.')}
        # 2. overwrite entries in the existing state dict
        model_dict.update(pretrained_dict)
        ...
        state_dict = torch.load(last_model_name)
        pretrained_dict = state_dict['a'] 
        model_dict = self.id_a.state_dict()
        # 1. filter out unnecessary keys
        ignore_layers = ['classifier1.classifier.0.weight', 'classifier1.classifier.0.bias', 'classifier2.classifier.0.weight', 'classifier2.classifier.0.bias']
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if k not in ignore_layers}
        # 2. overwrite entries in the existing state dict
        model_dict.update(pretrained_dict)
        self.id_a.load_state_dict(model_dict)
        self.id_b = self.id_a
        # Load optimizers
        try:
